...
function isNewGreeting(task: Task, now: number): boolean {
  return (
    task.lastServedAt === undefined && isSameCalendarDay(task.createdAt, now)
  );
}
...
export function pickNextCard(
  ...
  // 쪼개기 부모는 자식이 하나라도 안 끝났으면 후보에서 제외(engine-flow.md ③ — 자식 완료까지 대기).
  // 저장된 상태가 아니라 매번 derive: 자식 전부 done이면 다음 호출부터 자동으로 다시 후보가 됨.
  const hasUnfinishedChild = (t: Task) =>
    pool.some((c) => c.parentId === t.id && c.state !== "done");

  // §3.0 — 후보 = 활성 AND 재우기 해제 AND 쪼개기 자식 대기 중 아님(INV-6: dormant는 어떤 게이트/티어에도 등장 불가).
  const candidates = pool.filter(
    (t) =>
      t.state !== "done" &&
      (t.dormantUntil === undefined || t.dormantUntil <= now) &&
      !hasUnfinishedChild(t),
  );
  ...
  if (resuming.length > 0) {
    return {
      kind: "resume",
      task: pickBy(resuming, (a, b) => compareForResumeGate(a, b, now)),
    };
  }
  ...
  const splitting = candidates.filter(
    (t) => t.deferCount >= cfg.deferThreshold,
  );
  ...
  const ranked = candidates
    ...
    .sort((a, b) =>
      a.tier !== b.tier
        ? a.tier - b.tier
        : compareWithinTier(a.task, b.task, now),
    );
